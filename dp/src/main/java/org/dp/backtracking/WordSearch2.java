    ...
    private static boolean findWord(char[][] grid, String word) {
        ...
        for(int i = 0; i < grid.length; i++){
            for(int j = 0; j < grid[0].length; j++){
                if(findWord(grid, word, i, j, 0)) return true;
            }
        }
        return false;
    }

    private static boolean findWord(char[][] grid, String word, int i, int j, int pos) {
        ...
        for(int[] d : dir){
            if(findWord(grid, word, i + d[0], j + d[1], pos + 1)) {
                // restore state
                grid[i][j] = ch;
